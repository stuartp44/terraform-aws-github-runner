# Technical Review: GitHub Runner Job Hooks & Metrics
...
- Use any AWS service to deliver the required outputs to the user (explaining your choice)
- Add new files/scripts under `/tmp/runner`
- Access GitHub runner environment variables at job runtime (e.g. `RUNNER_NAME`, `GITHUB_RUN_ID`, etc.)

If you need to make reasonable assumptions, document them.

---

## 2. Task 1 – Pre-Job & Post-Job Scripts
...
- Create a per-run working directory, e.g. `/tmp/runner/workspace/$GITHUB_RUN_ID`
- Optionally export environment variables for the job

**Example expected behavior:** "Before every job starts, create a clean workspace directory for that run."
...
**Example expected behavior:** "After every job finishes, clean up temp data and write a small summary log."
...
- Choose a strategy to tie scripts to job start/end (you pick, but explain):
...
## 3. Task 2 – Collect Runtime Machine Metrics
...
Use a per-run directory such as: `/tmp/runner/$GITHUB_RUN_ID/`
...
Example: `startup.sh` (or equivalent)
